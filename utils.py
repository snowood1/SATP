...
# classes = ["False", "True"]
# classes = ["Assassination", "Hijacking", 'Kidnapping',	'Barricade Incident',	'Bombing/Explosion',
# 'Armed Assault', 'Unarmed Assault',	'Facility/Infrastructure Attack',	'Unknown']
# classes = [	'is_relevant', 'Armed Assault', 'Bombing/Explosion',  'Kidnapping', 'Other']
#
#
...
def read_csv(csv_folder, split, sentence_limit, word_limit, label_columns):
    """
    ...
    data = pd.read_csv(os.path.join(csv_folder, split + '.csv'), header=0)
    # print('\n\n hello >Z<,,,,>>>>>>>>>>>>')
    for i in tqdm(range(data.shape[0])):
        ...
        # labels.append(list(map(int,  row[1:])))  # since labels are 1-indexed in the CSV
        labels.append(list(map(int, [row[col] for col in label_columns])))
...
def create_input_files(csv_folder, output_folder, sentence_limit, word_limit, label_columns, min_word_count=5,
    ...
    train_docs, train_labels, word_counter = read_csv(csv_folder, 'train', sentence_limit, word_limit, label_columns)
    ...
    test_docs, test_labels, _ = read_csv(csv_folder, 'test', sentence_limit, word_limit, label_columns)
